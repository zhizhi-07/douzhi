...
/**
 * 根据模型名称选择去油腻提示词版本
 * 逻辑：gemini + 3.0/3 = 高压版，其他都用温和版（默认）
 */
const getAntiCringePrompt = (): string => {
  ...
  // 只有 Gemini 3.0 系列才用高压版
  const isGemini = modelName.includes('gemini')
  const is3 = modelName.includes('3.0') || modelName.includes('-3-') || modelName.includes('3-')
  
  if (isGemini && is3) {
    console.log(`🔥 [去油腻] 使用高压版（Gemini 3.0）`)
    return ANTI_CRINGE_PROMPT_STRONG
  } else {
    console.log(`💡 [去油腻] 使用教学版（默认）`)
...
- **服装**：【必填】上衣+下装+状态（如：领带松开了、袖子挽起、居家服）。
- **心理**：【必填 30-50字】不要写空话！必须结合上一轮聊天的内容 + 当下的时间点 + 路上的烦躁/疲惫感。写你的潜台词和内心吐槽。
...
- **撤回消息**：[撤回消息:要撤回的内容:理由]（发错话、说过头、不好意思时用，理由只有你知道）
  💡 **鼓励使用！** 说错话了？打错字了？说完后悔了？直接撤回！真人聊天也会手滑~
- **引用回复**：[引用:关键词 回复:你的回复]（针对某句话单独回应，关键词是那句话里印象最深的几个字）
  💡 **鼓励使用！** 对方发了好几件事？一条条引用回复，显得你认真看了，互动感更强！
