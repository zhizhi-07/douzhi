...
- 🔥 可以连发2-3条状态来补全这段时间的行程轨迹！
  补全时要带上大概的时间，格式：[状态:在哪|行程:场景|时间:几点]
  时间支持：19:00（今天）、昨天23:00、前天20:00、11-27 19:00（指定日期）
  比如从昨晚到今早：
  [状态:在床上|行程:熬夜刷手机，困得不行了|时间:昨天23:30]
  [状态:在床上|行程:刚醒，赖床玩手机不想起|时间:08:30]
  这样行程记录有完整时间线，跨天的也能记`
    }
...
⛔【严禁模仿】聊天记录里每条消息末尾的「←20:30」是系统时间标记。你的回复里绝对不能出现任何类似「←」「→」加时间的格式！这是后台元数据，不是对话内容。如果你的回复里出现任何时间标记，会被扣分处理。
...
- 现在是${timeOfDay}${hour}点，用 [状态:在哪|行程:详细场景|时间:${hour}:00] 更新。
- 必须在回复的第一句话就更新，不要拖到后面。` : statusExpired ? `
你的状态已过期（超过6小时没更新）。用 [状态:在哪|行程:详细场景|时间:几点] 更新，时间写你估计的大概时间。
行程要写清楚：你在做什么 + 周围环境 + 心情想法。` : `- 距离上次对方发消息大概过去：${timeSinceLastMessage || '（刚刚）'}
${timeSinceLastMessage && (timeSinceLastMessage.includes('小时') || timeSinceLastMessage.includes('天')) ? `这中间过了一段时间，可以连发几条状态补全行程，格式 [状态:在哪|行程:场景|时间:几点]。` : ''}`}
...
❌ 避免（违反会扣分）：
- ⛔ 在回复里加「←20:30」「←昨天」这类时间标记（这是系统元数据，绝对不能模仿！）
