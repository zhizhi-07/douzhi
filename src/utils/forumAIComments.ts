...
## 🏷️ 楼主标记了以下好友（⚠️ 这些是普通人，不是名人！）
**被标记的好友：${mentionedNormalCharacters.map(c => c.name).join('、')}**

${mentionedNormalCharacters.map(c => {
    return `- **${c.name}**【普通人】${c.personality ? `：${c.personality.substring(0, 100)}` : ''}`
  }).join('\n')}
...
**❌ 路人网友对被标记的普通好友的反应：**
- 路人网友**不认识**这些人，**完全无视**他们
- ❌ 禁止说"这不是xxx吗"、"大佬来了"、"名人"等
- 路人网友只关注帖子内容，不关心被@的普通人是谁
- 这就像你@了一个普通同学，其他网友不会围观讨论你同学是谁
` : ''
...
${normalCharacters.length > 0 ? `### 普通AI角色（⚠️ 不是名人！只是楼主的普通好友）
**重要：这些人不是公众人物！路人网友不认识他们！**
- 如果帖子@了他们，路人网友**不会**说"这不是xxx吗"、"大佬来了"
- 路人网友对他们没有任何特别反应，就像@了一个普通朋友一样
- 只有楼主和这个人本人知道彼此关系，其他网友不关心

${normalCharacters.slice(0, 5).map(a => {
    let info = `**${a.name}**【普通人，无人认识】`
    if (a.personality) info += `\n- 人设（只影响TA自己的说话风格）：${a.personality}`
    if (a.recentChat) {
      const chatLines = a.recentChat.split('\n').slice(-10)
      info += `\n- 【私密】与用户的聊天：\n${chatLines.map(l => '  ' + l).join('\n')}`
      // 添加时间对比信息
...
- 用词、称呼、话题都要符合世界观设定
- 例如：修仙世界用"道友"、"前辈"；赛博朋克用"老哥"、"赛博人"等
- 评论内容、语气、表情都要与世界观一致

` : ''

  // 获取用户数据（粉丝、关注、帖子、点赞）
  const { getUserData } = await import('./forumUser')
...
## 👤 楼主信息（发帖的人）
**楼主网名：「${userName}」**
**楼主数据：** 粉丝${userData.followers} | 关注${userData.following} | 帖子${userData.posts} | 获赞${userData.likes}
- 评论中提到楼主时，必须使用「${userName}」这个名字
- ❌ 禁止使用 "user"、"用户"、"楼主" 等代称，必须用真实网名「${userName}」
- 例如：应该写"${userName}太真实了"，而不是"user太真实了"
- 🔥 楼主是有${userData.followers}粉丝的博主，网友评论时可以自然提到"大V"、"博主"、"粉丝多"等
...
- 不要用明星名或AI角色的名字
- 每个名字只出现一次
- ❌ **禁止**：绝对不能生成代表楼主「${userName}」说话的内容！楼主主要以发帖为主，**大部分时间不亲自下场参与评论区对话**，让网友自己讨论
...
**如果@的是普通好友（非公众人物）：**
- 路人网友**不认识**这个人，**不会**围观讨论
- ❌ 禁止说"这不是xxx吗"、"大佬"、"名人"等
- 路人网友只关注帖子内容本身，不关心被@的普通人是谁
- 只有被@的本人会回复，其他网友不关心

- 被@的人本人可以选择回复或不回复（高冷人设可以不理）
- 评论区的焦点应该是**帖子内容**，而不是被@的普通人
...
- **数量要求**：必须生成 3-6 条私聊！
- **发送者比例**：
  - **80%是NPC网友**（普通路人、粉丝、好奇的陌生人等）
  - **20%是AI角色**（只有被@或帖子明确相关时才发）
- **NPC网友私聊**：用随机创意网名，如"深夜emo的猫"、"吃瓜小分队"、"神秘路人甲"等
...
[主楼] 路人甲：@楼主 这也太真实了
[回复] 吃瓜群众 -> 路人甲：同意
[私聊] 深夜失眠的猫：姐妹你这帖子太真实了哈哈哈，关注了！
[私聊] 神秘路人A：看到你发的了，想问一下那个是在哪买的？
[私聊] 吃瓜小能手：楼主你和那个谁什么关系啊，好好奇
[私聊] 热心网友小王：你好，我也遇到过类似的情况，可以交流一下吗

## 📊 帖子热度评估（必填！放在最后一行）
根据帖子内容质量、是否有公众人物参与、话题性等，评估这个帖子的热度：
**格式**：[热度] 点赞数=xxx 涨粉=xxx
- 点赞数：根据帖子质量决定（普通帖子2000-8000，有趣帖子8000-30000，热门帖子30000-100000，爆款帖子100000-500000+）
- 涨粉数：**必须大量涨粉！**（范围500-10000）
  - 普通帖子涨粉500-1500
  - 有趣帖子涨粉1500-3000
  - 热门帖子涨粉3000-6000
  - 爆款帖子涨粉6000-10000+
- 有公众人物参与或被标记 → 点赞和涨粉都要翻倍！

**示例**：
[热度] 点赞数=15000 涨粉=2200
[热度] 点赞数=180000 涨粉=8500
...
`
  ...
  try {
    const result = await callAIForCommentsBatch(actors, postContent, apiConfig, userPreviousPosts, mentionedPublicFigures, mentionedNormalCharacters, mentionedUserInfo, postAuthorInfo, imageUrls, currentUserName)
